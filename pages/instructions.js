          ...
          <List>
            <ListItem>
              <Typography variant="body2">
                ...
                include genus, specific epithet, and authority, if included. If
                the name is a subspecies or variety, the parser will also
                separate the rank indicator ("var.", "subsp.", "sbsp.", etc.)
                and the subspecific epithet. The parser also detects and
                separates standard botanical annotations such as "sp. nov." (new
                species) and "ined." (unpublished name) as well as indicators of
                uncertainty such as "cf." ("compare with") and "aff." (affinis,
                related to but not the same). Finally, any unrecognized
                components are saved as "Unmatched_Terms". Separating
            ...
            <ListItem>
              <Typography variant="body2">
                ...
                names with exactly the same match score. In such cases, the TNRS
                uses a conservative "Best Match Algorithm" to sort the names in
                descending order of match quality, preferring, for example,
                synonyms which have been corrected to a different accepted name
                over the same name labelled as accepted. After applying these
                rules, the TNRS marks the top-ranked name as the single best
                match. In such cases, the TNRS will alert you that multiple
                matches were found, allowing you to select an alternative match
                if preferred. We recommend that users examine all alternative
                matches rather than accepting uncritically the TNRS's choice of
                "Best Match".
              </Typography>
            </ListItem>
          </List>
          ...
          <List>
            ...
            <ListItem>
              <Typography variant="body2">
                ...
                Name Parse, then choose "Perform Name Resolution" under setting
                "Processing Mode". For "Family Classification", "Tropicos" uses
                APG III families for all matched and accepted names. Although
                Tropicos is currently the only available option, this may change
                in the future. For "Sources", some users may prefer or be
                required to use only a single taxonomic source. For example,
                government users in the U.S.A. may be required to resolve
                species names using USDA PLants taxonomy only.
              </Typography>
            </ListItem>
            <ListItem>
              <Typography variant="body2">
